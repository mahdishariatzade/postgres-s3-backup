...
for db in "${DBS[@]}"; do
  ...
  if [ -n "$db" ]; then
      ...
      # Use pigz for parallel compression if available (should be installed in Dockerfile)
      pg_dump -h "$POSTGRES_HOST" -p "$POSTGRES_PORT" -U "$POSTGRES_USER" -- "$db" | $GZIP_CMD | aws s3 cp - "${S3_DEST}/$FILE_NAME" "${AWS_ARGS[@]}"
